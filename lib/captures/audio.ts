...
/**
 * Streaming audio analysis for stereo OGG Opus captures. Decodes the audio
 * via ffmpeg-static into 16-bit little-endian PCM at 16 kHz and computes
 * per-channel RMS energy bins on the fly — full PCM never sits in memory.
 *
 * Captures are stereo by design — left (c0) = user mic, right (c1) = system
 * audio (other speakers). The energy bins drive `lib/captures/transcribe.ts`
 * dead-channel detection and `lib/captures/echo-leak.ts` echo suppression.
 */
...
/**
 * Per-channel RMS energy envelope. Used by `transcribeCapture` to
