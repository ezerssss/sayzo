...
- **mainIssueShape** — A transferable lesson the learner carries to their next attempt. Object with two short strings: `principle` and `shape`. This pairs with `mainIssue` (the diagnosis) and `fixTheseFirst` (the worked rewrite) to form three registers of the same lesson — abstract → form → concrete example.
    - `principle`: a short, quotable heuristic the learner should internalize. **One sentence, imperative or aphoristic, in plain language.** It must survive outside this drill — the learner should be able to recall it next week in a different meeting. Examples: *"Answer first, context on request."* / *"Commit before you qualify."* / *"Concrete over comprehensive."* / *"Pause where you'd type a comma."* / *"Name the trade-off, not the wish."* Forbidden: framework labels (*"use STAR"*, *"the Pyramid"*), generic platitudes (*"be clear"*, *"speak with confidence"*), or anything that names this specific drill.
    - `shape`: how that principle takes form **for this drill** — a 2–5 step skeleton or pattern in plain English, no framework names. Use `→` between steps. Examples: *"Recommendation → Trade-off → What you need from them"* / *"Position → One caveat (only if needed) → Stop"* / *"Pick one example → Walk through it → Skip the laundry list"* / *"Headline → Status → Risk → Next step"*. The shape should be specific enough to apply *on this kind of scenario* but plain enough that the learner doesn't need to memorize a label.
...
2. **`mainIssueShape.principle`** — the heuristic to internalize. One short, quotable line they could repeat to themselves before any future meeting. *"Answer first, context on request."* *"Commit before you qualify."* *"Concrete over comprehensive."* Plain-English imperatives, never framework names.
3. **`mainIssueShape.shape`** — the skeleton that principle takes here. A 2–5 step pattern with `→` separators, in their own domain language. *"Recommendation → Trade-off → What you need from them."*
...
`whyThisMatters` still names the **concrete listener cost** (*"the PM is rebuilding the picture from scratch"*). The principle is the reusable lesson; `whyThisMatters` is why that lesson exists. They're paired, not redundant.
