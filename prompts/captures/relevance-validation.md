...
4. **isEnglish** — The conversation is **majority English**. Sayzo only supports English. Set true when the bulk of the user-tagged speech is English, even if some non-English filler words or loanwords appear ("ciao", "gracias", "ja", brand names, etc. are fine). Set false when the majority of the user's substantive content is in another language. Borderline mixed-language conversations where English is at least half: accept.
...
- The majority of the user's substantive speech is in a language other than English (`isEnglish: false`).
...
- `isEnglish` (boolean)
- `rejectionReason` (string or null) — If rejecting, provide a clear one-sentence reason. If accepting, set to null. (For non-English rejections, the server overrides this with a standardized message; you can still populate it.)
