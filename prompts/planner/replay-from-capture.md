...
- **`scenario.category`** — pick the snake_case slug from the recommended catalog that best matches the actual conversation type (`status_update`, `meeting_contribution`, `difficult_conversation`, `stakeholder_alignment`, `demo_walkthrough`, etc.), or invent a new one in valid format if none fit.
