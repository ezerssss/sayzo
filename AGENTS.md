...
Per-user coaching state lives in ONE server-only `learner-models/{uid}` doc (access via `lib/learner-model/store.ts`). Per-item replay/conversation feedback shares one `ItemAnalysis` shape.
